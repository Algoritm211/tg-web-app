...
const getInvoice = ({id, title, userName, prices}: InvoiceProps) => {
  const invoice = {
    ...
    prices, // Price breakdown, serialized list of components in JSON format 100 kopecks * 100 = 100 rubles
...
const createInvoice = async (
  ...
  try {
    const {id, orderNumber, userName, items} = req.body
    ...
    // @ts-ignore
    await bot.telegram.sendInvoice(id, getInvoice({
      id,
      title: 'Ваш заказ',
      prices,
      userName: userName,
